...
<div class="content" style="margin-top: 40px">
    ...
    <form class="layui-form" action="" id="chapter-add-form">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="chapter_name" required lay-verify="required"
                       placeholder="请输入章标题" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-upload" style="margin-left: 110px;">
                <button type="button" class="layui-btn" id="chapter-add-form-file-commit">上传图片</button>
                <div class="layui-upload-list">
                    <img class="layui-upload-img" id="chapter-add-form-file-img-preview"
                         style="width: 150px;height: 150px;">
                    <p id="chapter-add-form-file-text"></p>
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit
                        lay-filter="chapter-add-form-commit">立即提交
                </button>
...
<script>
    (function ($) {
        layui.use(['form', 'upload'], function () {
            ...
            //监听提交
            form.on('submit(chapter-add-form-commit)', function (data) {
                $.ajax({
                    url: 'add.do',
                    data: data.field,
                    type: 'post',
            ...
            var uploadInst = upload.render({
                elem: '#chapter-add-form-file-commit' //绑定元素
                , field: 'img'
                , url: '<%=PATH%>/file/upload.do' //上传接口
                , before: function (obj) {
                    //预读本地文件示例，不支持ie8
                    obj.preview(function (index, file, result) {
                        $('#chapter-add-form-file-img-preview').attr('src', result); //图片链接（base64）
                    });
                }
                , done: function (res) {
                    if (res.success) {
                ...
                , error: function () {
                    //演示失败状态，并实现重传
                    var demoText = $('#chapter-add-form-file-text');
